...
async function runStage3Verify() {
  ...
  const userDataDir = path.join(__dirname, 'chrome-stage3-es173');
  ...
  try {
    const targets = await httpGet('http://localhost:9360/json');
    ...
    const outPath = path.join(__dirname, 'stage3_es173_home_customization.png');
