...
| 72H-RELEASE 72시간 실배포 (AI 프록시·모바일 뒤로가기·APK 파이프라인) | PR 병합 완료 | fix/2026-09-10-apk-build-jdk21 | #102 |
...
- **[필수] 커뮤니티 신고·자동숨김 + REST 노출 차단 SQL** (2026-09-11 재확인, 실행계획 순서 24): 클라이언트 코드(`report_content` RPC 호출, `hidden` 필터링)는 이미 완결돼 있으나 서버 스키마가 아직 한 번도 적용되지 않았다. Supabase SQL Editor에서 `docs/sql/2026-09-08-hidden-rls.sql` 1회 실행 필요(`2026-09-06-content-reports.sql`의 미실행분 포함). 실행 전에는 신고 버튼이 실패 토스트만 띄우고 아무것도 숨기지 못한다.
- **[필수] 사용자 차단(Block) 기능 SQL** (2026-09-11 재확인, 실행계획 순서 45): 클라이언트 코드(`blockUser`/`unblockUser`, 차단 목록 필터링, 설정 화면 UI)는 이미 완결돼 있으나 `user_blocks` 테이블이 없다. Supabase SQL Editor에서 `docs/sql/2026-09-10-ugc-safety-reports.sql` 1회 실행 필요. 실행 전에는 차단 버튼이 실패 토스트만 띄우고 저장되지 않는다.
- **[필수] Web Push 실사용 1회 테스트** (2026-09-11 재확인, 실행계획 순서 14): 구독 코드 경로(`openNotificationSoftAskModal` → `Notification.requestPermission` → `syncPushSubscription` → `/api/push-subscribe`)를 다시 읽어봤으나 결함을 찾지 못했다. 완료 기준이 계속 미충족인 이유는 순수하게 "설정에서 알림을 켠 계정이 0건"이라 발송이 한 번도 트리거되지 않은 것 — 코드로 재현·수정할 수 없는 실사용 블로커다. 아무 계정에서든 설정 > 알림 켜기를 최소 1회 눌러 실제 구독을 만들어야 다음 검증이 의미가 있다.
- **[필수] 공식 업무용 도메인 이메일** (실행계획 순서 47): `support@ourgoal.app` 등 공식 도메인 이메일 개설(도메인 구매·DNS 연결 선행) 전에는 `ysm0422@naver.com` 교체 작업을 코드로 진행할 수 없다.
