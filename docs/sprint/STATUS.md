...
- 현재 태스크: TASK-03
- 단계: PR 대기
- 브랜치: feat/2026-09-06-task-03-voice-checkin
- PR: #40
...
| TASK-03 Web Speech API 음성 체크인 & 퀵 루틴 | PR 대기 | feat/2026-09-06-task-03-voice-checkin | #40 |
...
- TASK-02 (병합 완료): 토스페이먼츠 개발자센터 가맹점(테스트) 등록·테스트 클라이언트 키 발급 — 이번 PR은 가상 성공 처리만 구현했으므로 이 설정 없이도 이미 동작 중. 다음 단계(실제 결제 승인)부터 필요.
- TASK-03 (PR 대기): 없음(브라우저 내장 API만 사용). iOS Safari 등 SpeechRecognition 미지원 브라우저에서는 마이크 버튼이 자동으로 숨겨짐.
