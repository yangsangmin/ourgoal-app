...
- **영향 받는 파일 목록 전수**:
  ...
  - `index.html`: [AI 목표 어시스턴트 카드 상단 전진 배치 및 데이터분석 프롬프트 백과사전 아코디언 접힘 개편]
...
- **[본질] (Engineering Essence)**:
  - 런타임 성능 저하(리플로우/리페인트 폭탄) 없이 CSS 레이아웃 엔진의 GPU 하드웨어 가속(Sticky 컴포지팅 계층)을 활용하여 모바일 375px 전 화면에서 무결한 네비게이션 가시성을 영구 확보하고, AI 어시스턴트 상단 전진 배치 및 프롬프트 백과사전 아코디언화로 연속적 유저 여정과 시각적 청정성을 극대화하는 것.
- **[원인] (Technical Causes)**:
  ...
  - AI 어시스턴트가 최하단에 위치하여 내 목표를 보며 작성할 수 없는 시각적 단절 및 프롬프트 백과사전 상시 펼침으로 인한 화면 과밀.
- **[중심 배선] (Core Wire & State)**:
  ...
  - `#goalAgentCard` 및 `_promptEncyclopediaOpen` 아코디언 토글 리스너의 온전한 배선.
- **[핵심 안전장치] (Critical Safety & Persistence)**:
  - 기존 클래스명과 DOM 위계를 일체 변경하지 않고 CSS 레이어와 렌더링 템플릿만 외과수술적으로 패치하여 기존 335개 테스트와 38개 무결성 게이트에 0건의 사이드이펙트를 보증.
...
| `index.html` | AI 어시스턴트 상단 이동 & 프롬프트 아코디언화 | +40줄 | -30줄 | +10줄 | UX 동선 최적화 |
| `js/sanctuary-v3-engine.js` | 목표 0건 알약 바 보존 렌더링 | +15줄 | -5줄 | +10줄 | 비파괴 마크업 보강 |
| `scripts/verify-integrity-gate.js` | #TASK-ES-201 정적 방화벽 검증 게이트 추가 | +55줄 | 0줄 | +55줄 | 회귀 차단 |
| `reports/TASK-ES-201/claims.json` | 법정 심사용 주장 파일 | +100줄 | 0줄 | +100줄 | 법정 제출 정본 |
