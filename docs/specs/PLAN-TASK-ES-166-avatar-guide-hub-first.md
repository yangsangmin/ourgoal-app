...
- **REQ 핵심 요약**:
  ...
  - 아바타 3대 혁신 기능(5대 상징 랭크 및 오라, 앱 진입 대형 인사 팝업 커스텀, 아바타 보관함) 및 전설의 우주 윙 달성자 비주얼 쇼케이스 카드 완비.
...
[설정 탭 클릭: #btnTabGuideHub] 
  └──> openTabGuideHubModal('avatar')
         ...
         │      ├──> 3대 혁신 기능 안내 (랭크 오라, 인사 팝업 멘트, 보관함)
         │      └──> Lv.20 우주 윙 달성자 비주얼 쇼케이스 카드 표출
         └──> [🎨 나만의 아바타 꾸미러 가기] 클릭
...
- [ ] **[1단계: 기획·설계 상태]**: REQ 및 PLAN 수립 완료.
- [ ] **[2단계: 내부 시뮬레이션 상태]**: 코드 구현 및 단위/스모크/게이트키퍼 테스트 통과.
- [ ] **[3단계: 로컬 수동 확인 상태]**: 로컬 환경에서 가이드 허브 아바타 탭 렌더링 및 클릭 동작 실측 확인.
- [ ] **[4단계: 로컬 메인 병합 상태]**: feature 브랜치를 로컬 main에 병합 (정상 마감점).
- [ ] **[5A단계: Vercel 프리뷰 배포]**: Vercel 임시 프리뷰 배포 자동 실행 및 상민님께 URL 제공.
