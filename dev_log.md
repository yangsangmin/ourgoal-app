...
참고: 위 장애는 사용자가 별도로 연 PR #19로 먼저 병합되어 해결되었고, 본 세션이 동시에 준비하던 동일 목적의 긴급 수정 PR #21은 중복이라 사용자가 직접 닫음. 아래 온보딩 투어 브랜치는 PR #21 브랜치를 베이스로 분기했던 터라, PR #19 병합 이후의 main과 병합 충돌이 발생해 이 로그를 포함한 3개 파일을 수동으로 재병합했다.

## [2026-09-05 12:33] 온보딩 직후 소통 기능 짧은 투어 추가
- **목표**: BACKLOG.md "진행 중/대기" 항목 — 첫 목표 생성 후 마니또·모임·공유 등 소통 기능을 자연스럽게 안내하는 짧은 투어/툴팁 추가.
- **[원칙 1~2] 문제 및 본질**: 신규 가입자는 온보딩(3단계: 카테고리 선택→목표 정하기→마일스톤 미리보기)을 마치면 곧바로 홈 화면으로 진입하는데, 이 앱의 핵심 차별점 중 하나인 소통 탭(마니또·모임·공유)은 스스로 탐색해서 찾아야 했다. 초반 이탈 방지와 "혼자가 아니다"는 인상을 주는 데는 첫 목표를 만든 그 순간이 가장 자연스러운 안내 시점이었다.
- **[원칙 3~4] 해결 방식 및 타당성 검토**: 새 투어 컴포넌트를 만들지 않고 기존 마니또 가입 화면에서 이미 쓰던 `.mission-card`/`.mi` 리스트 스타일을 그대로 재사용해 모달 1개로 구성(디자인 불변경 원칙 준수). 다크패턴 점검: "나중에 볼게요"로 언제든 건너뛸 수 있고 재노출·카운트다운·손실 강조 문구 없음. 온보딩 흐름 자체(3단계)는 전혀 바꾸지 않고, 마지막 단계(목표 생성 완료) 직후에만 추가 단계로 삽입 — 목표 생성을 건너뛰는 "다른 사람 목표 먼저 볼래요" 경로는 이미 소통 탭으로 바로 이동하므로 별도 투어를 붙이지 않음(중복 방지). 가입 시 1회만 실행되는 `startOnboarding()` 경로에만 넣었기 때문에 별도의 "본 적 있음" 플래그 없이도 자연스럽게 1회성이 보장됨.
- **[원칙 5~7] 구현 절차 및 검증 결과**: `showCommTourModal()` 신규 — "🎉 첫 목표를 만들었어요!" 헤드라인 + 마니또/모임/공유 3개 mission-card 요약 + "나중에 볼게요"/"소통 탭 보러가기"(클릭 시 소통 탭 피드로 바로 이동) 버튼. 온보딩 3단계(`showObStep3`)의 목표 생성 완료 콜백(`obFinish`) 끝, 기존 토스트 직후에 호출 1줄만 추가.
- **검증 결과**: `node -e`로 메인 `<script>` new Function() 문법 검증 통과, `node scripts/smoke-test.js` 28개 전부 통과(회귀 없음, 순수 UI 추가라 신규 단위 테스트 대상 없음). `.mission-card`/`.mi` 클래스가 기존에 정의돼 있어 신규 CSS 0줄임을 grep으로 확인. 브라우저 도구가 없는 샌드박스라 실제 온보딩 플로우 클릭 테스트는 코드 리뷰로 대체(온보딩 3단계 완료 콜백 안에 정확히 삽입됐음을 diff로 확인).
- **[원칙 8] 재검증**: 해당 없음(막힌 지점 없었음). 이 브랜치도 긴급 수정 PR #21 위에서 분기.
