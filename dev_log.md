...
## [2026-09-08 00:15] 페이월 캘린더 혜택 문구 정직화
- **목표**: BACKLOG.md "페이월 캘린더 혜택 문구 정직화"(PR #54 리뷰어 지적) 처리. Pro 페이월의 캘린더 혜택 행이 실제 동작과 다른 문구("캘린더 양방향 실시간 동기화 · 항상 최신 상태로")를 쓰고 있어 사실대로 고친다.
- **문제 및 본질(원칙1~2)**: 실제 캘린더 기능은 항목별 📅 버튼을 눌러 목표·마일스톤 일정을 구글 캘린더로 수동 전송하는 것뿐이고(index.html:1682 안내문 "목표와 마일스톤 일정을 구글 캘린더에 보낼 수 있어요(항목별 📅 버튼)"), 자동 양방향 실시간 동기화는 존재하지 않는다. 페이월(index.html:1743)만 이 사실과 다른 과장 문구를 쓰고 있었다.
- **해결 방식 및 타당성(원칙3~4)**: 새 문구를 만들지 않고 이미 검증된 정직한 안내문(1682번 줄)과 같은 표현을 재사용해 "목표·마일스톤 일정을 구글 캘린더로 보내기 · 항목별 버튼 한 번으로"로 교체. mission-row 구조·CSS 클래스·아이콘은 그대로 두어 CLAUDE.md 2번(디자인 불변경)과 충돌 없음. 다른 Pro 혜택 항목이나 calendarAvailable 게이팅 로직은 건드리지 않아 기존 기능 삭제도 없음.
- **구현 절차 및 검증(원칙5~7)**: index.html 1743번 줄 문자열 리터럴 1곳만 diff 수정. `new Function()` 문법 검증 통과, `node scripts/smoke-test.js` 66/66 통과(회귀 없음, 이번 변경은 테스트 대상 함수 아님).
- **재검증 내역(원칙8)**: 해당 없음 — 막힌 지점 없이 진행.
- **검증 결과**: 문법✅ 스모크 66/66✅ (배포 검증은 PR 병합 후 Vercel 프리뷰에서 확인 필요 — UI 문자열 변경이라 브라우저 렌더 확인 권장하나 이번 세션은 브라우저 도구 미사용, 코드 검토로 대체)
---
