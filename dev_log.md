...
참고: 위 장애는 사용자가 별도로 연 PR #19로 먼저 병합되어 해결되었고, 본 세션이 동시에 준비하던 동일 목적의 긴급 수정 PR #21은 중복이라 사용자가 직접 닫음. 이 브랜치(체크인 전체 내보내기)는 PR #21 브랜치를 베이스로 분기했던 터라, PR #19 병합 이후의 main과 병합 충돌이 발생해 위 로그를 포함한 3개 파일을 수동으로 재병합했다.

## [2026-09-05 12:32] 데이터 내보내기 개선 (체크인 전체 이력 CSV/JSON)
- **목표**: BACKLOG.md "진행 중/대기" 항목 — 설정의 내보내기가 요약 위주였는데 체크인 전체 이력을 CSV/JSON으로 온전히 내려받을 수 있게 보강.
- **[원칙 1~2] 문제 및 본질**: 조사해보니 기록 탭의 "AI 분석용 파일로 내보내기"(`exportRecordsBtn`)는 이미 `state.profile.records` 전체를 슬라이싱 없이 CSV/JSON으로 내보내고 있어 실제로는 "전체 이력"이었다. 반면 목표 상세의 "AI 분석용 내보내기"(`buildGoalSnapshot`)는 `recentCheckins.slice(0,60)`으로 의도적으로 60개까지만 담는 요약본(AI 컨텍스트용이라 적절함)이었다. 진짜 공백은 **설정(Settings) 화면의 "데이터" 섹션에는 체크인 전용 내보내기가 없고, 프로필 전체를 통째로 JSON 백업하는 "전체 내보내기" 버튼만 있어서 CSV로 스프레드시트에 바로 열어보고 싶은 사용자가 설정 화면에서는 그 방법을 찾을 수 없었다는 것**이었다. 부가로, 체크인에 최근 추가된 `category` 필드가 CSV 내보내기에는 반영되지 않고 있었다.
- **[원칙 3~4] 해결 방식 및 타당성 검토**: 새 내보내기 인프라를 만들지 않고 기존 `buildCSV()`/`exportRecordsBtn` 로직을 `exportAllCheckins()` 함수로 추출해 재사용하는 방식을 택함(중복 코드 없이 설정 화면에서도 동일 동작 제공). `buildCSV()`에는 `분야` 컬럼만 추가(TOPICS 라벨). 기존 "전체 내보내기" 버튼은 이름을 "전체 백업"으로 바꿔 새 "체크인 전체 내보내기"와 성격을 명확히 구분(둘 다 유지, 기능 삭제 없음). CLAUDE.md 디자인 불변경 원칙 점검: 버튼 1개 추가 + 안내 문구 1줄 추가일 뿐 기존 `.data-btn-row`/`.btn-ghost` 레이아웃·스타일은 그대로.
- **[원칙 5~7] 구현 절차 및 검증 결과**:
  (1) `buildCSV()`에 `분야` 컬럼 추가(`TOPICS[r.category].label`, 없으면 빈 칸).
  (2) 기록 탭의 익명 클릭 핸들러를 `exportAllCheckins()` 이름 있는 함수로 추출(체크인 개수를 알려주는 토스트로 개선).
  (3) 설정 화면 "데이터" 블록에 `#settingsExportCheckins`("체크인 전체 내보내기") 버튼 신규 추가, `exportAllCheckins()`에 연결. 기존 `#settingsExportAll`은 라벨만 "전체 백업"으로 변경(기능 동일). 두 버튼의 차이를 설명하는 안내 문구 1줄 추가.
- **검증 결과**: `node -e`로 메인 `<script>` new Function() 문법 검증 통과, `<style>` 중괄호 407/407(CSS 변경 없음), `node scripts/smoke-test.js` 28개 전부 통과(회귀 없음). `buildCSV()` 로직을 Node로 격리 시뮬레이션해 분야 컬럼이 정확히 포함되고 기존 컬럼 순서·이스케이프 처리가 깨지지 않음을 확인. 브라우저 도구가 없는 샌드박스라 실제 다운로드 클릭 동작은 로직 검증으로 대체.
- **[원칙 8] 재검증**: 해당 없음(막힌 지점 없었음). 다만 이 브랜치는 이번 사이클에 발견한 긴급 수정 PR(#21, main의 병합 충돌 마커로 인한 프로덕션 문법 오류)을 베이스로 분기해 작업했음 — PR 설명에 명시.
