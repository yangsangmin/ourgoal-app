...
## [2026-09-08 00:30] 랜딩 부트 블록 함수 분리 + 게이트 순서 테스트 (AUD-8)
- **목표**: BACKLOG.md "랜딩 부트 블록 함수 분리 + 게이트 순서 테스트"(AUD-8) 처리. `landing_view` 하루 1회 게이트가 부트 IIFE 안에 익명으로 묶여 있어 회귀를 스모크로 고정할 수 없던 결함을 없앤다.
- **문제 및 본질(원칙1~2)**: `boot()` IIFE 안에서 `getAttribution()` 호출 → `localStorage.getItem('ourgoal_lv_day')` 비교 → `track('landing_view', ...)` 순서가 이름 없는 인라인 블록으로 존재해(index.html 옛 7072~7076줄), 이 로직만 따로 실행하거나 단위 테스트로 고정할 방법이 없었다(AUD-1이 지적한 "게이트 로직이 부트 안에 매몰"되는 결함 유형과 동일). 이미 병합된 PR #48·#49·#51·#52·#54가 손대지 않은 영역이라 충돌 없이 착수 가능해졌다.
- **해결 방식 및 타당성(원칙3~4)**: `track()` 함수 바로 뒤에 순수에 가까운 `recordLanding(todayKey)`를 추가 — 오늘 이미 기록했으면 `false`를 반환하고 아무 것도 하지 않고, 아니면 `ourgoal_lv_day`를 갱신하고 `track('landing_view', attrib)`을 호출한 뒤 `true`를 반환한다. `boot()` 안의 5줄 인라인 블록은 `recordLanding(dateKey(nowISO()))` 한 줄 호출로 치환 — 동작 순서·조건·예외 처리(try/catch로 계측 실패 무시)는 문자 그대로 보존해 기존 기능 변경이 없다. CLAUDE.md 2번(디자인 무관, 로직 전용)·3번(diff 단위 수정)과 정합.
- **구현 절차 및 검증(원칙5~7)**: index.html에 함수 추가(+10줄) + boot() 5줄 → 1줄 치환. `scripts/smoke-test.js`: `recordLanding`을 FN_NAMES에 추가, 실제 Supabase 연동 없이 호출 여부만 보려고 `track()`을 호출 기록 배열에 담는 스텁으로 대체(기존 navigator/badge 스텁과 같은 패턴), 테스트 2건 추가 — "lv_day가 이미 오늘이면 재기록 안 함(호출 0건)" / "오늘이 아니면 1회 기록+attrib 포함, 같은 날 재호출 시 추가 기록 안 됨". 검증: `new Function()` 문법 통과, `node scripts/smoke-test.js` **68/68** 통과(기존 66 + 신규 2), `git diff` 삭제 줄은 전부 치환(순서·조건문 동일), 기존 boot() 흐름(세션 있으면 로그인 화면 숨김 등)은 무변경.
- **재검증 내역(원칙8)**: 해당 없음 — 막힌 지점 없이 진행.
- **검증 결과**: 문법✅ 스모크 68/68✅ (배포 검증은 PR 병합 후 Vercel에서 랜딩 화면 최초 로드 시 `events` 테이블에 `landing_view` 1건이 하루 1회만 쌓이는지 확인 권장 — 이번 세션은 브라우저 도구 미사용, 순수 함수 단위 테스트로 게이트 로직을 고정)
---
