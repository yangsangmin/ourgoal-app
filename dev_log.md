...
## [2026-09-06 01:07] PR 병합 규칙(CLAUDE.md 6번) 채팅 승인 예외 추가
- **목표**: PR #26(타임존 스모크 테스트 수정) 병합을 사용자가 채팅에서 지시했으나, "PR을 스스로 병합하지 않는다"는 기존 문구 때문에 Claude가 자체 판단으로 병합을 보류함 — 사용자가 "내가 병합하라고 하거나 작업시작부터 자동 병합하라고 하면 채팅으로도 병합 가능하게" 규칙 변경을 지시.
- **수정/실행 내역**: CLAUDE.md §6의 병합 규칙을 "원칙적으로 사용자가 직접" + "단, 채팅에서 특정 PR 병합을 명시 지시하거나 사이클 시작 시 '자동 병합하라'고 선언한 경우 Claude가 `gh pr merge`로 직접 병합"하는 예외로 수정. 병합 직후 8-B의 충돌 마커 확인 + 스모크 테스트 수행을 그대로 유지. 이 예외는 자동화 루틴과 대화형 세션 모두에 적용됨을 명시.
- **발생한 문제 및 해결**: 실제 `gh pr merge` 실행은 Claude Code 자체의 자동 모드 안전 분류기가 별도로 차단할 수 있음(CLAUDE.md로 제어 불가한 영역) — 이 사실과 "차단 시 재시도하지 말고 사용자에게 직접 병합을 요청" 방침을 규칙에 참고 사항으로 병기함.
- **검증 결과**: CLAUDE.md 텍스트 diff 검토 완료(문법 검증 대상 아님, 순수 문서 변경).
---
