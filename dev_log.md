...
## [2026-09-08 00:55] docs/sql/*.sql 문법 검사를 스모크에 추가 (AUD-7)
- **목표**: BACKLOG.md "`docs/sql/*.sql` 문법 검사를 스모크에 추가"(AUD-7) 처리. 새 테이블·RPC SQL이 어떤 DB에서도 한 번도 실행되지 않은 채 PR이 열리는 구조라, 최소한 명백한 오탈자(괄호 안 닫힘, `$$` 안 짝맞음, `create policy`에 `on` 절 누락)는 병합 전에 잡는다.
- **문제 및 본질(원칙1~2)**: `docs/sql/*.sql`은 사람이 Supabase SQL Editor에 수동으로 붙여넣어야 적용되는데, 이 저장소의 스모크 테스트는 index.html만 검증하고 SQL 파일은 전혀 검사하지 않는다. 진짜 실행 검증(DB에 실제로 돌려보기)은 Supabase 프로젝트 접근이 필요해 이번 세션 범위 밖이지만, 문법 수준의 명백한 실수는 파일 텍스트만으로도 잡을 수 있다.
- **해결 방식 및 타당성(원칙3~4)**: 외부 SQL 파서 라이브러리를 새로 들이지 않고(의존성 추가는 이번 스코프에서 과함) 정규식 기반 최소 검사 `lintSql(sql)`을 `scripts/smoke-test.js`에 직접 작성 — (1) `$$` 개수 짝수 (2) 괄호 짝(단, `$$...$$` 함수 본문 내부는 PL/pgSQL이라 바깥과 괄호·세미콜론 규칙이 다르므로 통째로 제외하고 검사) (3) `create policy` 뒤에 `on <table>` 절이 있는지(따옴표 있는/없는 정책 이름 모두 허용) (4) 마지막 문장이 세미콜론으로 끝나는지. 완전한 SQL 검증이 아님을 주석에 명시해 오해를 막았다. CLAUDE.md와 무관한 순수 도구 스크립트라 디자인·기존 기능과 충돌 없음.
- **구현 절차 및 검증(원칙5~7)**: `scripts/smoke-test.js`에 `[3/3]` 섹션 추가(기존 `[1/2]`·`[2/2]` 라벨을 `[1/3]`·`[2/3]`로 갱신), `docs/sql/` 아래 4개 파일 전부에 대해 통과 여부를 개별 `check()`로 리포트 + `lintSql` 자체의 단위 테스트 3건(괄호 오류 검출·`on` 누락 검출·`$$` 함수 본문 내부는 통과). 처음 돌렸을 때 `docs/sql/2026-09-06-events.sql`의 `create policy events_insert_client on public.events`(따옴표 없는 정책 이름)가 오탐(false positive)으로 걸려, 정규식을 `"[^"]+"|\S+`로 넓혀 따옴표 유무 모두 허용하도록 수정 — 실제 버그를 하나 고친 셈. 검증: `node scripts/smoke-test.js` **74/74** 통과(기존 66 + 신규 8: 파일 존재 1 + SQL 파일별 4 + lintSql 단위 테스트 3).
- **재검증 내역(원칙8)**: 위 이벤트 SQL 오탐 발견 시 1~5단계로 돌아가 정규식을 수정(원칙 8 그대로 적용된 사례).
- **검증 결과**: 문법✅ 스모크 74/74✅(이 브랜치 기준 — PR #76의 새 SQL 파일과 합쳐지면 5개 파일 전부 검사 대상이 됨, 별도 확인 완료)
---
