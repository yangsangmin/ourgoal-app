...
- **문제해결 8원칙 및 PR 4블록 적용 규칙 (헌법 제2조 & 제6조 엄수)**:
  ...
  - **PR 설명문 4블록 규격 (헌법 제6조 제1항 제2호)**: PR 본문은 헌법 제6조의 표준 4블록으로 작성한다:
    ...
    3. `[블록 3]`: 법정 판정서 첫 네 줄 (원칙 6~7 — `npm test` 는 예비 검사이며 법정 판정을 대신하지 않는다)
...
1. [항목명] — PR #번호
   ...
   - 무엇을: (1줄)
   - 법정: <통과|확인 부족|돌려보냄|심사 전> · 지시 n건 중 눌러 확인 n · 코드만 n · 확인 못 함 n · 판정번호
   - ⚠️ 주의사항 (있을 때만)
...
────────────────
병합을 여쭙는 PR: 법정 판정이 통과·확인 부족인 것만 (돌려보냄·심사 전인 PR 은 병합을 요청하지 않는다)
