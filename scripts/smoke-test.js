...
check('compliance: [#TASK-ES-281] 소통탭 게시하기 버튼 먹통 오류 수정 및 정상 동작 복구', () => {
  ...
  assert.ok(commJs.includes('handle소통_Item30Action = handle소통_Item30Action'), 'handle소통_Item30Action 노출');

  // 2. index.html DOM 마크업 검증
  assert.ok(indexHtml.includes('id="btnCommPostFeed"'), 'index.html #btnCommPostFeed 게시하기 버튼 탑재');
  assert.ok(indexHtml.includes('handle소통_Item30Action'), 'index.html #btnCommPostFeed에 handle소통_Item30Action 연동');
  assert.ok(indexHtml.includes('id="og-task-30-container"'), 'index.html #og-task-30-container 마크업 탑재');
  assert.ok(indexHtml.includes('id="og-task-30-action-btn"'), 'index.html #og-task-30-action-btn 버튼 마크업 탑재');
  assert.ok(indexHtml.includes('handle소통_Item30Action(event)'), 'index.html 액션 버튼 onclick 핸들러 탑재');

  // 3. ui.css 스타일 및 반응형 검증
  assert.ok(uiCss.includes('#og-task-30-container'), 'ui.css #og-task-30-container 스타일 정의');
  assert.ok(uiCss.includes('#og-task-30-action-btn'), 'ui.css #og-task-30-action-btn 스타일 정의');
  assert.ok(uiCss.includes('min-width: 44px;'), 'ui.css 버튼 최소 터치 폭 44px 정의');
  assert.ok(uiCss.includes('min-height: 44px;'), 'ui.css 버튼 최소 터치 높이 44px 정의');
});
...
check('compliance: [#TASK-ES-287] 동반자 탭 내 AI 동반자 전면 제거 (실 사용자 중심 전환)', () => {
  ...
  assert.ok(compJs.includes('handle팀목표_Item37Action = handle팀목표_Item37Action'), 'handle팀목표_Item37Action 노출');
  assert.ok(compJs.includes('og_task-37_cache'), 'og_task-37_cache 원자적 영속화 키 정의');
  assert.ok(teamJs.includes('handle팀목표_Item37Action'), 'team-invite-comm.js 내 handle팀목표_Item37Action 노출');

  // 2. index.html 컨테이너 및 액션 버튼 검증
  assert.ok(indexHtml.includes('id="og-task-37-container"'), 'index.html #og-task-37-container 마크업 탑재');
  assert.ok(indexHtml.includes('id="og-task-37-action-btn"'), 'index.html #og-task-37-action-btn 버튼 마크업 탑재');
  assert.ok(indexHtml.includes('handle팀목표_Item37Action(event)'), 'index.html 액션 버튼 onclick 핸들러 탑재');

  // 3. ui.css 스타일 및 반응형 검증
  assert.ok(uiCss.includes('#og-task-37-container'), 'ui.css #og-task-37-container 스타일 정의');
  assert.ok(uiCss.includes('#og-task-37-action-btn'), 'ui.css #og-task-37-action-btn 스타일 정의');
  assert.ok(uiCss.includes('min-width: 44px;'), 'ui.css 버튼 최소 터치 폭 44px 정의');
  assert.ok(uiCss.includes('min-height: 44px;'), 'ui.css 버튼 최소 터치 높이 44px 정의');
});
...
check('TASK-ES-288: 설정창 전면 개편 4위 1체 배선 및 모바일 375px 규격 검증', () => {
  ...
  assert.ok(compJs.includes('async function handle전체공통_Item38Action('), 'handle전체공통_Item38Action 함수 정의');
  assert.ok(compJs.includes('handle전체공통_Item38Action = handle전체공통_Item38Action'), 'handle전체공통_Item38Action 노출');
  assert.ok(compJs.includes('og_task-38_cache'), 'og_task-38_cache 원자적 영속화 키 정의');
  assert.ok(custJs.includes('handle전체공통_Item38Action'), 'customize.js 내 handle전체공통_Item38Action 노출');

  // 2. index.html 컨테이너 및 액션 버튼 검증
  assert.ok(indexHtml.includes('id="og-task-38-container"'), 'index.html #og-task-38-container 마크업 탑재');
  assert.ok(indexHtml.includes('id="og-task-38-action-btn"'), 'index.html #og-task-38-action-btn 버튼 마크업 탑재');
  assert.ok(indexHtml.includes('handle전체공통_Item38Action(event)'), 'index.html 액션 버튼 onclick 핸들러 탑재');

  // 3. ui.css 스타일 및 반응형 검증
  assert.ok(uiCss.includes('#og-task-38-container'), 'ui.css #og-task-38-container 스타일 정의');
  assert.ok(uiCss.includes('#og-task-38-action-btn'), 'ui.css #og-task-38-action-btn 스타일 정의');
  assert.ok(uiCss.includes('min-width: 44px;'), 'ui.css 버튼 최소 터치 폭 44px 정의');
  assert.ok(uiCss.includes('min-height: 44px;'), 'ui.css 버튼 최소 터치 높이 44px 정의');
});
...
check('TASK-ES-290: 일정 배경사진 최대 2장 및 상하 반반 분할 레이아웃 4위 1체 배선 검증', () => {
  ...
  assert.ok(compJs.includes('async function handle목표탭_Item40Action('), 'handle목표탭_Item40Action 함수 정의');
  assert.ok(compJs.includes('handle목표탭_Item40Action = handle목표탭_Item40Action'), 'handle목표탭_Item40Action 노출');
  assert.ok(compJs.includes('og_task-40_cache'), 'og_task-40_cache 원자적 영속화 키 정의');
  assert.ok(goalRegistryJs.includes('formatScheduleBackgroundLayout'), 'goal-templates-registry.js 내 formatScheduleBackgroundLayout 노출');

  // 2. index.html 컨테이너 및 액션 버튼 검증
  assert.ok(indexHtml.includes('id="og-task-40-container"'), 'index.html #og-task-40-container 마크업 탑재');
  assert.ok(indexHtml.includes('id="og-task-40-action-btn"'), 'index.html #og-task-40-action-btn 버튼 마크업 탑재');
  assert.ok(indexHtml.includes('handle목표탭_Item40Action(event)'), 'index.html 액션 버튼 onclick 핸들러 탑재');

  // 3. ui.css 스타일 및 반응형 검증
  assert.ok(uiCss.includes('#og-task-40-container'), 'ui.css #og-task-40-container 스타일 정의');
  assert.ok(uiCss.includes('#og-task-40-action-btn'), 'ui.css #og-task-40-action-btn 스타일 정의');
  assert.ok(uiCss.includes('.schedule-bg-split-container'), 'ui.css .schedule-bg-split-container 스타일 정의');
  assert.ok(uiCss.includes('.schedule-bg-split-item'), 'ui.css .schedule-bg-split-item 스타일 정의');
  assert.ok(uiCss.includes('min-width: 44px;'), 'ui.css 버튼 최소 터치 폭 44px 정의');
  assert.ok(uiCss.includes('min-height: 44px;'), 'ui.css 버튼 최소 터치 높이 44px 정의');
});
...
check('TASK-ES-291: 레벨업 연출 멘트 및 배경 변경 유도 4위 1체 배선 검증', () => {
  ...
  assert.ok(compJs.includes('async function handle아바타_Item41Action('), 'handle아바타_Item41Action 함수 정의');
  assert.ok(compJs.includes('handle아바타_Item41Action = handle아바타_Item41Action'), 'handle아바타_Item41Action 노출');
  assert.ok(compJs.includes('og_task-41_cache'), 'og_task-41_cache 원자적 영속화 키 정의');
  assert.ok(avatarJs.includes('triggerAvatarLevelUpDialogue'), 'avatar-system.js 내 triggerAvatarLevelUpDialogue 노출');
  assert.ok(avatarJs.includes('진짜 잘했다! 내자신! 내 뒤의 배경좀 바꿔줘라 지겹다!'), 'avatar-system.js 내 대사 정의');

  // 2. index.html 컨테이너 및 액션 버튼 검증
  assert.ok(indexHtml.includes('id="og-task-41-container"'), 'index.html #og-task-41-container 마크업 탑재');
  assert.ok(indexHtml.includes('id="og-task-41-action-btn"'), 'index.html #og-task-41-action-btn 버튼 마크업 탑재');
  assert.ok(indexHtml.includes('handle아바타_Item41Action(event)'), 'index.html 액션 버튼 onclick 핸들러 탑재');

  // 3. ui.css 스타일 및 반응형 검증
  assert.ok(uiCss.includes('#og-task-41-container'), 'ui.css #og-task-41-container 스타일 정의');
  assert.ok(uiCss.includes('#og-task-41-action-btn'), 'ui.css #og-task-41-action-btn 스타일 정의');
  assert.ok(uiCss.includes('.avatar-levelup-dialogue-bubble'), 'ui.css .avatar-levelup-dialogue-bubble 스타일 정의');
  assert.ok(uiCss.includes('min-width: 44px;'), 'ui.css 버튼 최소 터치 폭 44px 정의');
  assert.ok(uiCss.includes('min-height: 44px;'), 'ui.css 버튼 최소 터치 높이 44px 정의');
});
...
check('TASK-ES-293: 성취통계 측정지표 다중선택 필터링 4위 1체 배선 검증', () => {
  ...
  assert.ok(compJs.includes('async function handle성취통계_Item43Action('), 'handle성취통계_Item43Action 함수 정의');
  assert.ok(compJs.includes('handle성취통계_Item43Action = handle성취통계_Item43Action'), 'handle성취통계_Item43Action 노출');
  assert.ok(compJs.includes('toggleAchievementMetricFilter'), 'toggleAchievementMetricFilter 함수 정의');
  assert.ok(compJs.includes('og_task-43_cache'), 'og_task-43_cache 원자적 영속화 키 정의');

  // 2. index.html 컨테이너 및 액션 버튼 검증
  assert.ok(indexHtml.includes('id="og-task-43-container"'), 'index.html #og-task-43-container 마크업 탑재');
  assert.ok(indexHtml.includes('id="og-task-43-action-btn"'), 'index.html #og-task-43-action-btn 버튼 마크업 탑재');
  assert.ok(indexHtml.includes('handle성취통계_Item43Action(event)'), 'index.html 액션 버튼 onclick 핸들러 탑재');

  // 3. ui.css 스타일 및 반응형 검증
  assert.ok(uiCss.includes('#og-task-43-container'), 'ui.css #og-task-43-container 스타일 정의');
  assert.ok(uiCss.includes('#og-task-43-action-btn'), 'ui.css #og-task-43-action-btn 스타일 정의');
  assert.ok(uiCss.includes('.metric-multiselect-chip'), 'ui.css .metric-multiselect-chip 스타일 정의');
  assert.ok(uiCss.includes('min-width: 44px;'), 'ui.css 버튼 최소 터치 폭 44px 정의');
  assert.ok(uiCss.includes('min-height: 44px;'), 'ui.css 버튼 최소 터치 높이 44px 정의');
});
...
check('TASK-ES-295: 성취통계 데이터 관리 옆 접기토글 4위 1체 배선 검증', () => {
  ...
  // 1. js/components.js 정의 및 export 검증
  assert.ok(compJs.includes('async function handle성취통계_Item45Action('), 'handle성취통계_Item45Action 함수 정의');
  assert.ok(compJs.includes('handle성취통계_Item45Action = handle성취통계_Item45Action'), 'handle성취통계_Item45Action 노출');
  assert.ok(compJs.includes('toggleDataManagementSection'), 'toggleDataManagementSection 함수 정의');
  assert.ok(compJs.includes('og_task-45_cache'), 'og_task-45_cache 원자적 영속화 키 정의');

  // 2. index.html 컨테이너 및 액션 버튼 검증
  assert.ok(indexHtml.includes('id="og-task-45-container"'), 'index.html #og-task-45-container 마크업 탑재');
  assert.ok(indexHtml.includes('id="og-task-45-action-btn"'), 'index.html #og-task-45-action-btn 버튼 마크업 탑재');
  assert.ok(indexHtml.includes('handle성취통계_Item45Action(event)'), 'index.html 액션 버튼 onclick 핸들러 탑재');

  // 3. ui.css 스타일 및 반응형 검증
  assert.ok(uiCss.includes('#og-task-45-container'), 'ui.css #og-task-45-container 스타일 정의');
  assert.ok(uiCss.includes('#og-task-45-action-btn'), 'ui.css #og-task-45-action-btn 스타일 정의');
  assert.ok(uiCss.includes('min-width: 44px;'), 'ui.css 버튼 최소 터치 폭 44px 정의');
  assert.ok(uiCss.includes('min-height: 44px;'), 'ui.css 버튼 최소 터치 높이 44px 정의');
});
...
check('TASK-ES-301: 피드 게시 모달 내 미리보기 버튼 및 피드 렌더링 사전 확인 4위 1체 배선 검증', () => {
  ...
  // 1. js/components.js 정의 및 export 검증
  assert.ok(compJs.includes('async function handle소통_Item51Action('), 'handle소통_Item51Action 함수 정의');
  assert.ok(compJs.includes('handle소통_Item51Action = handle소통_Item51Action'), 'handle소통_Item51Action 노출');
  assert.ok(compJs.includes('openFeedPostPreviewModal'), 'openFeedPostPreviewModal 함수 정의');
  assert.ok(compJs.includes('toggleFeedPostPreview'), 'toggleFeedPostPreview 함수 정의');
  assert.ok(compJs.includes('og_task-51_cache'), 'og_task-51_cache 원자적 영속화 키 정의');

  // 2. index.html 컨테이너, 액션 버튼 및 피드 미리보기 슬롯 검증
  assert.ok(indexHtml.includes('id="og-task-51-container"'), 'index.html #og-task-51-container 마크업 탑재');
  assert.ok(indexHtml.includes('id="og-task-51-action-btn"'), 'index.html #og-task-51-action-btn 버튼 마크업 탑재');
  assert.ok(indexHtml.includes('handle소통_Item51Action(event)'), 'index.html 액션 버튼 onclick 핸들러 탑재');
  assert.ok(indexHtml.includes('id="sharePreviewBtn"'), 'index.html 피드 게시 모달 내 sharePreviewBtn 탑재');
  assert.ok(indexHtml.includes('id="sharePreviewSlot"'), 'index.html 피드 게시 모달 내 sharePreviewSlot 탑재');
  assert.ok(indexHtml.includes('window.toggleFeedPostPreview = toggleFeedPostPreview'), 'index.html window.toggleFeedPostPreview 노출');

  // 3. ui.css 스타일 및 반응형 검증
  assert.ok(uiCss.includes('#og-task-51-container'), 'ui.css #og-task-51-container 스타일 정의');
  assert.ok(uiCss.includes('#og-task-51-action-btn'), 'ui.css #og-task-51-action-btn 스타일 정의');
  assert.ok(uiCss.includes('#sharePreviewSlot'), 'ui.css #sharePreviewSlot 스타일 정의');
  assert.ok(uiCss.includes('#sharePreviewBtn'), 'ui.css #sharePreviewBtn 스타일 정의');
  assert.ok(uiCss.includes('min-width: 44px;'), 'ui.css 버튼 최소 터치 폭 44px 정의');
  assert.ok(uiCss.includes('min-height: 44px;'), 'ui.css 버튼 최소 터치 높이 44px 정의');
});
...
check('TASK-ES-302: 팀 목표 탭 최초 진입 시 접을 수 있는 모든 아코디언 기본 접힘 4위 1체 배선 검증', () => {
  ...
  // 1. js/components.js 정의 및 export 검증
  assert.ok(compJs.includes('async function handle팀목표_Item52Action('), 'handle팀목표_Item52Action 함수 정의');
  assert.ok(compJs.includes('handle팀목표_Item52Action = handle팀목표_Item52Action'), 'handle팀목표_Item52Action 노출');
  assert.ok(compJs.includes('collapseAllTeamGoalAccordions'), 'collapseAllTeamGoalAccordions 함수 정의');
  assert.ok(compJs.includes('og_task-52_cache'), 'og_task-52_cache 원자적 영속화 키 정의');

  // 2. index.html 컨테이너, 액션 버튼 및 팀 목표 기본 접힘 검증
  assert.ok(indexHtml.includes('id="og-task-52-container"'), 'index.html #og-task-52-container 마크업 탑재');
  assert.ok(indexHtml.includes('id="og-task-52-action-btn"'), 'index.html #og-task-52-action-btn 버튼 마크업 탑재');
  assert.ok(indexHtml.includes('handle팀목표_Item52Action(event)'), 'index.html 액션 버튼 onclick 핸들러 탑재');
  assert.ok(indexHtml.includes('collapseAllTeamGoalAccordions();'), 'index.html renderTeamGoalsScreen 내 collapseAllTeamGoalAccordions 호출');
  assert.ok(indexHtml.includes('window.collapseAllTeamGoalAccordions = collapseAllTeamGoalAccordions'), 'index.html window.collapseAllTeamGoalAccordions 노출');

  // 3. ui.css 스타일 및 반응형 검증
  assert.ok(uiCss.includes('#og-task-52-container'), 'ui.css #og-task-52-container 스타일 정의');
  assert.ok(uiCss.includes('#og-task-52-action-btn'), 'ui.css #og-task-52-action-btn 스타일 정의');
  assert.ok(uiCss.includes('min-width: 44px;'), 'ui.css 버튼 최소 터치 폭 44px 정의');
  assert.ok(uiCss.includes('min-height: 44px;'), 'ui.css 버튼 최소 터치 높이 44px 정의');
});
