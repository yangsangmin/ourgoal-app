...
- **[DEV-15] 자동 소환·재검증으로 받은 지시라면 열린 조직 변경 PR을 먼저 확인한다**(`gh pr list --state open --search "chore org"`). 이번 마찰을 이미 다루는 PR이 있으면 전수 재검증을 돌리지 않고 해제조건 1개만 실측한 뒤 그 PR에 `재발 n회차 · <ISO 시각> · 상태 불변` 코멘트 1줄을 남기고 종료한다(ORG.md §7-1-1). 병합 전 규칙을 실제로 적용하는 것이 아니라 병합 판단 근거를 쌓는 것이다.
